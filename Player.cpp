...
std::vector<SnakeSegment> Player::GetSnakeBody()
{
  return m_SnakeBody;
}
...
PlayerDirection Player::GetDirection()
{
  ...
  if(Head.Position.x == Neck.Position.x)
    ...
    return (Head.Position.x >Neck.Position.x ? PlayerDirection::Right : PlayerDirection::Left);
...
{
  m_Score += 10;
  m_ScoreLog->Add("Score: " + std::to_string((int)m_Score));
}
...
void Player::Reset()
{
  ...
  m_ScoreLog->Add("Score: " + std::to_string((int)m_Score));
  m_LivesLog->Add("Lives: "+std::to_string((int)m_Lives));
}
